...
import FormDialog3 from './dialog3';
const initialValue = { name: "", imageUrl: "", jobtitle: "", section: "", reward: "", BonusValue: "" ,weekenddays: '',Alternativetransport:''}
...
function Reward() {
  ...
  const handleClose = () => {
    ...
    const url = "http://localhost:4000/reward"
  ...
  const  columns=[
    { title: 'id', field: 'id' },
        ...
        render: rowData => <div>
          ...
          <input type="image" alt="" src={rowData.imageUrl} style={{width: 40, borderRadius: '50%'}}/> 
          <span className={rowData.status ? "on" : "off"}></span>
          </div>},
    ...
    return (
      ...
      <MaterialTable style={{marginTop:'100px'}}
        ...
        data={query =>
          new Promise((resolve, reject) => {
            ...
            let url = 'http://localhost:4000/reward?'
            //searching
            if (query.search) {
              url += `q=${query.search}`
            }
      ...
      />
       ...
       <FormDialog3 open={open} handleClose={handleClose}
